...
import project_codes.utils.joint_transforms as joint_transforms
import project_codes.utils.transforms as extended_transforms
from project_codes.datasets import cityscapes
from project_codes.models import *
from project_codes.utils import check_mkdir, evaluate, AverageMeter, CrossEntropyLoss2d, FocalLoss2d
import torch
...
args = {
    ...
    'snapshot': '',  # empty string denotes no snapshot, type in the name of the pth file to start training on that.
    'print_freq': 20,
    'val_batch_size': 4,
...
def main():
    ...
    net = UNet(output_ch=cityscapes.num_classes)      # import the network
    net = net.to(device=device)                          # put the network to GPU
...
#    criterion = CrossEntropyLoss2d(reduction='mean', ignore_index=cityscapes.ignore_label).cuda()

    criterion = FocalLoss2d(reduction='mean', ignore_index=cityscapes.ignore_label).cuda()
    ...
    open(os.path.join(ckpt_path, exp_name, 'test' + '.txt'), 'w').write(str(args) + '\n\n')
